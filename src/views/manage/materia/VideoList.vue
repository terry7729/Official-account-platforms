  ...
  <card title="素材管理" class="h-100">
    ...
    <el-row>
      <el-col :xs="8" :sm="10" :md="12" :lg="14" :xl="16">
        <el-button type="primary" class="el-icon-plus" @click="$router.push({name: 'AddVideo'})">上传视频</el-button>
      </el-col>
      <el-col :xs="16" :sm="14" :md="12" :lg="10" :xl="8">
        <el-input placeholder="搜索视频标题" v-model="searchVal" class="search-input">
      ...
      <el-table
        ...
        <el-table-column show-overflow-tooltip>
          <template slot="header" slot-scope="scope">
            <span class="table-title">全部视频</span><span class="info-text" v-show="scope">（共 {{ data.total }} 条）</span>
          </template>
          <template slot-scope="scope">
            <div class="no-wrap pos-r">
              <el-image
                style="width: 100px; height: 90px; vertical-align: middle;"
                :src="scope.row.sourceMaterialVideoIcon"
                fit="fill">
              </el-image>
              <div class="video-duration tc">
                <i class="el-icon-caret-right"></i>
                {{ $funs.getVideoTime(scope.row.recTime) }}
              </div>
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete(index, row) {
        this.$confirm('删除后不会影响已群发的图文消息，确定删除该素材？', '删除确认', {
